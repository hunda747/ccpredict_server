...
# Load the pre-trained model outside the request handler
model = tf.keras.models.load_model('./mobile_net_cancer.h5')
...
def preprocess_image(image):
    ...
    image = (image - 0.5) / 0.5  # Rescale pixel values to [-1, 1] (if needed)
...
def predict():
    ...
    # Return the predictions as a JSON response
    return jsonify({'predicted_class': predicted_class, 'probabilities': predictions.tolist()})
